...
3. **默认单币种 + 点击展开全部持仓**
   - 默认显示一个主仓（优先 BTC/ETH/SOL/OKX，按持仓量选最大）；
   - 点击按钮后可查看当前全部持仓明细；
   - 卡片展示标记价格。

4. **最近爆仓价面板**
   - 卡片上方展示 ETH/BTC/SOL/OKX 的最近爆仓价；
   - 增加“多空博弈”比例条（条形长度按多空占比动态变化）；
   - 没有数据显示 `--`。
...
10. **右上角排行筛选下拉**
   ...
   - 选择“配置”会回到 config.json 交易员；选择“所有”会合并配置+所有分类并去重后展示；其他选项会拉取对应 follow-rank 临时带单员列表；
